...
class NfcProxy {
  ...
  writeNdef = withAndroidPrompt(async ({type, value}: {type: any; value: any}) => {
    let result = false;
  ...
  customTransceiveNfcA = withAndroidPrompt(async (commands: any, onPostExecute: any) => {
    let result = false;
    const responses = [];
      ...
      try {
        ...
        for (const {type, payload} of commands) {
          ...
          if (type === 'command') {
            console.log(
              '>>> ' +
                payload.map((b: any) => ('00' + b.toString(16)).slice(-2)).join(' '),
            );
  ...
  customTransceiveNfcV = withAndroidPrompt(async (commands: any, onPostExecute: any) => {
    let result = false;
    const responses = [];
